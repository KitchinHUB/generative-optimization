...
{>~ @jk we need to consider moving this to another section, maybe in SI or the repo. My concern is we won't have this level of detail for flow matching and it will be imbalanced.~<}
...
We sample a range of rate constants and compute the exit molar flows of A and B. This data set is what we will use to build a generative model next.  We generate samples of $k_1, k_2$ in the range of 0.1..3. We can see what this data looks like here.

{>~ @jk  need a figure here ~<}
...
It is also possible to estimate an uncertainty of sorts. We reconsider the case where there is a low molar flow of A (0.02) with low molar flow of B (0.05) at the exit. We note "of sorts" because in this case there is no noise in the data, and the spread observed is related to deficiencies in the models.
