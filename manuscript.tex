...
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./fig_gmm_example.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./fig_fm_example.png}
...
To find the roots, we condition on \(r(Z) = 0\) and sample from the resulting conditional distribution \(p(Z \mid r = 0)\). Figure \ref{fig:pr-roots} shows the conditional probability density, which exhibits peaks corresponding to the liquid (\(Z_L \approx 0.035\)), unstable (\(Z \approx 0.13\)), and vapor (\(Z_V \approx 0.81\)) roots. Cluster analysis of the samples yields:
...
\begin{figure}[H]
\centering
\includegraphics[width=.9\linewidth]{./preos_comparison.png}
...
Rather than solving the optimality condition analytically, the generative approach starts by generating training data consisting of tuples \(\left(\tau, C_B(\tau), \frac{dC_B}{d\tau}\right)\), learning the joint distribution \(p\left(\tau, C_B, \frac{dC_B}{d\tau}\right)\), then identifying the optimum by sampling from the conditional distribution \(p\left(\tau \mid \frac{dC_B}{d\tau} = 0\right)\). This probabilistic inference approach generalizes to complex reaction networks where analytical solutions do not exist.
...
Inequality constraints bring a new challenge to using a generative method; there could be a range of possible values that could satisfy the solution (that is the nature of inequality), so it is not possible to condition on one set of values like it is with the equality constraint. A benefit of the conventional \texttt{minimize} function in \texttt{scipy} is one simply changes the constraint type.
...
Rearranging gives the equilibrium residual:
...
For a concrete example, we use a series reaction: A \(\rightarrow\) B \(\rightarrow\) C in a plug flow reactor. The observations we have will be the exit molar flow rates of A and B. We use an inlet molar flow, fixed volume and volumetric flow rate. The system is governed by these two differential equations:
