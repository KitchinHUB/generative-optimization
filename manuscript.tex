...
From a mathematical standpoint, we construct the joint probability distribution over all process variables $\boldsymbol{x}$, the equality constraint residuals $\boldsymbol{e}$, and the KKT stationarity residuals $\nabla_x L$. For the GMM approach, we learn $p(\boldsymbol{x}, \boldsymbol{e}, \nabla_x L)$ and condition on $[\boldsymbol{e} = \boldsymbol{0}, \nabla_x L = \boldsymbol{0}]$ to obtain candidate solutions satisfying both feasibility and first-order optimality; flow matching similarly generates $\boldsymbol{x}$ conditioned on full KKT residuals being zero. To generate realistic training data, we adopt an \emph{operating modes mixture} approach: Mode 1 ($\sim$60\%) solves barrier-augmented feasibility problems with widely varying constraints representing normal operations; Mode 2 ($\sim$30\%) solves actual cost minimization problems representing efficiency-focused periods, naturally producing points near constraint boundaries where optima lie; and Mode 3 ($\sim$10\%) explores tight constraint bands near target boundaries. This mixture ensures coverage of both diverse operating points and the optimal region.
