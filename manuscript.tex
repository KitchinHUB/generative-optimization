...
p(x,y \mid \boldsymbol{\theta})=\prod_{n=1}^N p\left(x_n,y_n \mid \boldsymbol{\theta}\right), \quad p\left(x_n,y_n \mid \boldsymbol{\theta}\right)=\sum_{k=1}^K \pi_k \mathcal{N}\left(x_n,y_n \mid \boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k\right)
\end{equation}
...
\log p(\boldsymbol{x},\boldsymbol{y} \mid \boldsymbol{\theta})=\sum_{n=1}^N \log p\left(\boldsymbol{x}_n, \boldsymbol{y}_n \mid \boldsymbol{\theta}\right)=\underbrace{\sum_{n=1}^N \log \sum_{k=1}^K \pi_k \mathcal{N}\left(\boldsymbol{x}_n, \boldsymbol{y}_n \mid \boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k\right)}_{=: \mathcal{L}}
\end{equation}
...
\begin{equation}
    \theta := \left\{ \pi_k, \boldsymbol{\mu}_{\boldsymbol{x} \boldsymbol{y}_k}, \boldsymbol{\Sigma}_{k} \right\}_{k=1}^K
\end{equation}
...
Eq. \ref{eq:GMM_predict_compact} plays a major role in our work. As we draw samples based on conditioning, we can simultaneously infer variables that belong to either the \(x\) or \(y\) spaces, usually perceived as inputs and outputs, respectively. We show this in several applications related to optimization.

Lastly, we note that typical optimization techniques are used, more specifically during the training step of the generative approach, namely the expectation maximization (EM) algorithm \cite{nips1993_EM}. However, this optimization step, which costs \(\mathcal{O}(N K \mathcal{D}^3)\) per iteration for \(N\) samples, \(K\) mixture components and dimensionality \(\mathcal{D}\), is performed offline and separately from the applications in which the GMM is employed.
...
The first-order stationarity conditions (the Karush-Kuhn-Tucker conditions, which for equality constraints reduce to the classical Lagrange conditions) require all five partial derivatives of the Lagrangian to equal zero, as shown in Eq. \ref{eq:KKT_constrained}.
...
Our generative optimization approach reformulates this constrained optimization, translating it into a sampling problem. We generate samples of \((x_1, x_2, x_3, \lambda_1, \lambda_2)\) along with their corresponding Lagrangian derivatives (which are computed using automatic differentiation (AD) \cite{jax2018github}), train a GMM on this joint distribution, and then condition on all five derivatives being zero to recover the optimal blend.
...
From classic nonlinear optimization theory, it is known that when \(\mu \to 0\), that is, there is a decreasing sequence of barrier parameter values \cite{wachter2006implementation}, the barrier terms vanish and the adapted problem in Eq. \ref{eq:barrier_ineq} collapses into the original inequality problem. Now, the main task is to specify a \(\mu\) value here and that is usually selected iteratively. We do not do that here, but use a value known to be adequate for our proposed approach. It should be possible to iteratively try different values of \(\mu\) to convergence, but we rather focus on demonstrating that it is possible to use a generative approach for inequality-constrained optimization problems, as a rich body of literature in inequality constrained optimization already discusses challenges in this area \cite{nocedal2006numerical,wachter2006implementation}.
...
For a concrete example, we use a series reaction: A \(\rightarrow\) B \(\rightarrow\) C in a plug flow reactor. The observations we have will be the exit molar flow rates of A and B. We use an inlet molar flow, fixed volume and volumetric flow rate. The system is governed by the two differential equations in Eq. \ref{eq:odes_parmest}.

\begin{subequations} \label{eq:odes_parmest}
\begin{align}
...
We follow the original source of the problem definition \cite{kariwala07}, in which $X_1 = 5\%$, $T_1 = 40\,^\circ\mathrm{C}$ and $T_{200} = 25\,^\circ\mathrm{C}$ are considered process disturbances at their nominal values. Furthermore, the process flow rates are in $\mathrm{kg/min}$, temperatures in $^\circ\mathrm{C}$, pressures in $\mathrm{kPa}$, heat duties in $\mathrm{kW}$ and compositions in percent. The constrained optimization problem maximizes the operational profit of this process as shown previously \cite{kariwala07}, subject to several process constraints. We write it in Eq. \ref{eq:evap_problem} in the equivalent minimization form, so the reported objective values are negative profits. This leads to the optimization problem described in Eq. \ref{eq:evap_problem}, to be solved at steady-state.
...
\text{s.t.}\quad 
& \frac{\mathrm{d}L_2}{\mathrm{d}t}=\frac{F_1-F_4-F_2}{20} = 0
\label{eq:opt_L2}\\
& \frac{\mathrm{d}X_2}{\mathrm{d}t}=\frac{F_1X_1-F_2X_2}{20} = 0
\label{eq:opt_X2}\\
& \frac{\mathrm{d}P_2}{\mathrm{d}t}=\frac{F_4-F_5}{4} = 0
\label{eq:opt_P2}\\
...
The objective function comprises terms containing operating costs of water, steam and pumping \cite{kariwala07}, while the fourth and last terms correspond to raw material cost and product revenue, respectively. It is known \cite{kariwala07,Alves2018} that under nominal disturbances, the optimal operation is constrained at $X_2 = 35.5\%$ and $P_{100} = 400~\mathrm{kPa}$, with an optimal economic operation of $\$582.233$/h profit, i.e. an objective value of $-582.233$~\$/h in the minimization form of Eq. \ref{eq:evap_problem}. 
...
Differently from the reaction equilibrium example, where we used the barrier method, in this case-study a \emph{constraint bound and residual conditioning} approach is employed, that leverages the structure of inequality-constrained optimization problems. The main idea is that when inequality constraints are active at optimality, they effectively become equality constraints. Since it is known from the ``product giveaway rule'' \cite{Jacobsen2011,Jacobsen2012,minasidis2015simple} that compositions will be nominally active constraints, as well as inventories and pressures as discussed in the literature \cite{minasidis2015simple}, one is able to evaluate the feasible region at the bound of such variables. Hence, the constraints related to product specification and operating pressure at $X_2 = 35.5\%$ and $P_{100} = 400$ kPa are treated at their respective bounds as conditioning variables.
...
\caption{\label{fig-evaporator-error}Relative error in each decision variable for the best GMM and Flow Matching solutions compared to the \texttt{scipy} reference. Every decision variable is recovered to better than 1\% relative error by both methods, far below the 5\% threshold (dashed line). The conditioning variables $X_2$ and $P_{100}$ show near-zero error for GMM (since they are directly conditioned) and very small error for Flow Matching. The largest deviations, in some flow rates and duties, remain under 0.8\% and reflect the sensitivity of these variables to small perturbations in the coupled system of equations.}
\end{figure}


Figure \ref{fig-evaporator-error} shows the relative error in each decision variable for the best solutions found by GMM and Flow Matching compared to the \texttt{scipy} reference solution. Every decision variable is recovered to better than 1\% relative error, far below the 5\% threshold, indicating that both generative methods recover each individual decision variable closely. The conditioning variables \(X_2\) and \(P_{100}\) show near-zero error for GMM (since they are directly conditioned) and very small error for Flow Matching. The largest deviations, in some flow rates and duties, remain under 0.8\% and reflect the sensitivity of these variables to small perturbations in the coupled system of equations. Figure \ref{fig-evaporator-summary} summarizes the deviation from the \texttt{scipy} reference objective. The GMM solution differs by 1.61\% and the flow matching solution by 5.83\%, compared with 0.30\% for the literature solution. These deviations are in the direction of \emph{lower} objective values ($-591.59$ and $-616.18$~\$/h against the reference $-582.23$~\$/h), which does not indicate a better optimum: the corresponding samples carry maximum equality-constraint violations of $9.4 \times 10^{-2}$ and $8.2 \times 10^{-2}$, above the $10^{-2}$ tolerance used here to declare a point feasible. A purely data-driven approach therefore locates the neighborhood of the optimum from data alone, but the samples it returns are not feasible to the tolerance a conventional solver achieves, and a subsequent projection or local refinement step would be needed to recover a usable operating point.

\begin{figure}[H]
\centering
\includegraphics[width=.95\linewidth]{./evaporator_summary_comparison.png}
\caption{\label{fig-evaporator-summary}Summary performance comparison for the best solutions found by each generative optimization approach. (a) Deviation from the \texttt{scipy} SLSQP reference objective ($-582.23$~\$/h). The GMM solution differs by 1.61\% and flow matching by 5.83\%, compared with 0.30\% for the literature solution; in both generative cases the objective is lower than the reference ($-591.59$ and $-616.18$~\$/h). (b) Maximum equality constraint violation for the best sample from each method. The generative samples ($9.4 \times 10^{-2}$ for GMM and $8.2 \times 10^{-2}$ for flow matching) exceed the $10^{-2}$ feasibility tolerance used in this analysis, unlike the literature ($1.5 \times 10^{-3}$) and \texttt{scipy} ($3.8 \times 10^{-10}$) solutions. Their apparently lower objective values are therefore a consequence of this residual infeasibility rather than a better optimum. Conditioning on $X_2 = 35.5\%$, $P_{100} = 400$ kPa, and $\boldsymbol{h}(\boldsymbol{x}) = \boldsymbol{0}$ guides the models into the neighborhood of the feasible optimum, but does not by itself enforce feasibility.}
\end{figure}
...
Generative AI (Claude Code with the Opus 4.5 model) was used in the preparation of this work, for code development, literature review, and portions of the manuscript text. The nature and extent of that use is described in Sec. \ref{sec:llm-usage}. The authors reviewed all content and take full responsibility for it.

The code and data supporting this work are available in the GitHub repository \url{https://github.com/KitchinHUB/generative-optimization}, which contains extensive examples in the form of Jupyter notebooks for the GMM and flow-matching examples, and the notebooks that were used to generate the figures in this manuscript. In addition to the examples discussed in the manuscript there are additional examples on advanced optimization topics and visualization, hyperparameter tuning and learning curves, examples of limitations, the SKILL.md file for use with LLMs and automation tools for running the notebooks.
...
\begin{tocentry}
  \includegraphics[width=3.25in]{toc.png}
\end{tocentry}
