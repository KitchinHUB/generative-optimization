...
\noindent Rather than solving the optimality condition analytically, the generative approach starts by generating training data consisting of tuples \(\left(\tau, C_B(\tau), \frac{dC_B}{d\tau}\right)\), learning the joint distribution \(p\left(\tau, C_B, \frac{dC_B}{d\tau}\right)\), then identifying the optimum by sampling from the conditional distribution \(p\left(\tau \mid \frac{dC_B}{d\tau} = 0\right)\) (Figure \ref{fig-optimization}). Furthermore, it is important to note that this probabilistic inference approach theoretically generalizes to any complex reaction networks where analytical solutions do not exist, including more complex applications in which a first principles model is not readily available.

\begin{figure}[H]
\centering
\includegraphics[width=.9\linewidth]{./optimization_comparison.png}
\caption{\label{fig-optimization}Optimization via conditional sampling for consecutive reactions A \(\rightarrow\) B \(\rightarrow\) C. (a) Intermediate product concentration \(C_B(\tau)\) shows maximum at \(\tau^* \approx 3.05\) min. (b) First derivative \(dC_B/d\tau\) crosses zero at the optimum. (c-d) GMM and flow matching identify the optimum by sampling from \(P(\tau C_B(\tau) | dC_B/d\tau = 0)\), producing narrow distributions (red/blue solid lines show means) that closely match the analytical solution (black dashed line). Both methods successfully solve the optimization problem through probabilistic inference, demonstrating that conditioning on the optimality condition (derivative = 0) yields the optimal residence time with inherent uncertainty quantification.}
\end{figure}
...
For \(K_{eq} = 4\), the analytical solution where \(f(\xi) = 0\) is \(\xi^* = 1/2\). The physical constraint that concentrations must be non-negative requires \(0 < \xi < 1\).
...
Given a radius, we have \(V = \pi R^2 H\) and given an \(RT\) we have these rate constants for the two forward reactions: \(k_1 = \exp(-3 / RT)\), \(k_2 = \exp(-10 / RT)\). We start with a mole balance:
...
\noindent which we can solve with the quadratic equation to get \(C_A\) as a function of \((R, RT)\), and then from a mole balance on B, we get
\begin{equation}
C_B = k_1 C_A^2 V / \nu_0
\end{equation}
