...
#        make clean        - Remove checkpoint and LaTeX scratch files
...
# LaTeX scratch files
#
# Every build drops a pile of aux/log/fls/... next to the sources.  This
# directory is inside Dropbox, so anything left behind gets synced and
# eventually comes back as a "conflicted copy", which is how stale junk
# ends up in the repo.  Each build target therefore removes its own
# scratch files on the way out.
#
# manuscript.bbl and manuscript.pdf are deliberately NOT in this list:
# the zip target ships the .bbl so recipients need not run bibtex.
# acs-manuscript.bib is generated by achemso, not written by hand.
# ------------------------------------------------------------------
...
TEXTRASH = $(foreach e,$(TEXEXT),manuscript.$(e) manuscript-diff.$(e)) \
	   manuscript-diff.bbl acs-manuscript.bib acs-manuscript-diff.bib
...
manuscript-diff.pdf: manuscript.tex manuscript.bib $(FIGURES)
	...
	@git show $(DIFFBASE):manuscript.tex > .manuscript-base.tex
	@latexdiff .manuscript-base.tex manuscript.tex > manuscript-diff.tex
	@rm -f .manuscript-base.tex
	latexmk -pdf manuscript-diff.tex
	@$(MAKE) --no-print-directory texclean
...
clean: texclean
	...
	@rm -f $(ZIPNAME)
	@echo "Checkpoint and LaTeX scratch files removed!"
...
help:
	...
	@echo "  texclean  - Remove LaTeX scratch files (aux, log, fls, ...)"
	@echo "  clean     - Remove checkpoint files and LaTeX scratch files"
